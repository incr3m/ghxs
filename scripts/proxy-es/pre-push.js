...
const { getApiDirectory, readJson, amplifyDir } = require('./lib/common');
...
async function transformApi() {
  const gqlApiDirectory = getApiDirectory();
  ...
  for (const key in searchableStack.Resources) {
    if ('ElasticSearchStreamingLambdaIAMRole' === key) {
    ...
    } else if ('ElasticSearchStreamingLambdaFunction' === key) {
      ...
      _.set(searchableStack, `Resources.${key}.Properties.Code`, { ZipFile: ES_CODE });
